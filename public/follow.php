...
<div>
  <?= htmlspecialchars($followee_user['username']) ?> さんをフォローしました。<br>
  <a href="/profile.php?user_id=<?= $followee_user['id'] ?>">
    <?= htmlspecialchars($followee_user['username']) ?> さんのプロフィールへ
  </a>
  /
  <a href="/users.php">
    会員一覧へ
...
<?php else: ?>
<div>
  <?= htmlspecialchars($followee_user['username']) ?> さんをフォローしますか?
  <form method="POST">
    <button type="submit">
      フォローする
    </button>
